...
use super::functions::*;
use super::keymap::{Action, Key, Resolved};
...
fn _run(mut state: State, session_path: PathBuf) -> Result<(), FxError> {
    ...
    // Keys typed so far toward a possible multi-key chord (e.g. the first `g`
    // of `gg`). Cleared once a chord resolves or dead-ends.
    ...
    'main: loop {
        ...
        match event::read()? {
            Event::Key(KeyEvent {
                ...
                pending.push(key);
                let visual = state.v_start.is_some();
                let action = match state.keymap.resolve(&pending, visual) {
                    // A live prefix (e.g. the first `g` of `gg`): wait for more.
                    Resolved::Prefix => continue,
                    // Dead end: drop the buffer and start over.
                    Resolved::None => {
                        pending.clear();
                        continue;
                    }
                ...
                match action {
                        ...
                        if let Some(start_pos) = state.v_start {
                            ...
                            for _n in 0..half {
                                if state.layout.nums.index == 0 {
                                    break;
                                } else if state.layout.y - cursor_move_count <= BEGINNING_ROW + 3
                    ...
                    //Go to top (second `g` of `gg` resolved by the keymap)
                    Action::GoToTop => {
                        if let Some(start_pos) = state.v_start {
                            //visual mode
                            if state.layout.nums.index == 0 {
                            ...
                            }) = event::read()?
                            {
                                match (code, modifiers) {
                                    ...
                                    | (KeyCode::Char('h'), KeyModifiers::CONTROL) => {
                                        ...
                                        command.remove((current_pos - INITIAL_POS_Z - 1).into());
                                        current_pos -= 1;
                                    ...
                                    (KeyCode::Char(c), _) => {
                                        command.insert((current_pos - INITIAL_POS_Z).into(), c);
                                        current_pos += 1;
                                        clear_current_line();
                                        to_info_line();
                                        print!("{}", &command.iter().collect::<String>(),);
                    ...
                    //insert mode
                    Action::EnterInsert | Action::EnterInsertDir => {
                        //In visual mode, this is disabled.
                        if state.v_start.is_some() {
                            continue;
                        }
                        let is_dir = matches!(action, Action::EnterInsertDir);
                        delete_pointer();
                        go_to_info_line_and_reset();
                        if is_dir {
                        ...
                        'insert: loop {
                            ...
                            }) = event::read()?
                            {
                                match (code, modifiers) {
                                    ...
                                    (KeyCode::Left, KeyModifiers::NONE) => move_left_command_line(
                                        &mut new_name,
                    ...
                    Action::ToggleSplit => match state.layout.split {
                        Split::Vertical => {
                    ...
                    //delete (second `d` of `dd` resolved by the keymap)
                    Action::Delete => {
                        //If read-only, deleting is disabled.
                        if state.is_ro {
                            print_warning("Cannot delete item in this directory.", state.layout.y);
                            continue;
                        }
                        if state.v_start.is_some() {
                            //visual mode
                            ...
                            if let Err(e) = state.delete(None, false, &mut screen) {
                                print_warning(e, state.layout.y);
                                continue;
                            }
                        }
                    ...
                    //yank (second `y` of `yy` resolved by the keymap)
                    Action::Yank => {
                        if state.v_start.is_some() {
                            //visual mode
                            ...
                            if let Ok(item) = state.get_item() {
                                state
                                    .registers
                                    .yank_item(&[ItemBuffer::new(item)], None, false);
                                state.escape();
                    ...
                    Action::Put => {
                        ...
                        if let Err(e) = state.put(state.registers.unnamed.clone(), &mut screen) {
                            print_warning(e, state.layout.y);
                        }
                    }
                                    ...
                                    (KeyCode::Char(c), _) => {
                                            ...
                                            unicode_width::UnicodeWidthChar::width(c)
                                        {
                                            ...
                                            let target = match state.ignore_case {
                                                Some(true) => state.list.iter().position(|x| {
                                                    x.file_name
                                                        .to_lowercase()
                        ...
                        'reg: loop {
                            ...
                            }) = event::read()?
                            {
                                match code {
                                    ...
                                    KeyCode::Right => {
                                        if current_pos as usize
                                            == command.len() + INITIAL_POS_COMMAND_LINE as usize
                                        {
                                            continue;
                                        };
                                        current_pos += 1;
                                    ...
                                    KeyCode::Backspace => {
                                        if current_pos == INITIAL_POS_COMMAND_LINE {
                                            ...
                                            command.remove(
                                                (current_pos - INITIAL_POS_COMMAND_LINE - 1).into(),
                                            );
                                            current_pos -= 1;
                                    ...
                                    KeyCode::Char(c) => {
                                        ...
                                        if ((state.v_start.is_some() || c == 'p')
                                            && command.len() == 2)
                                            || (state.v_start.is_none() && command.len() == 3)
                                        {
                                            ...
                                            match action.as_str() {
                                                ...
                                                "p" => {
                                                    ...
                                                    let target = match command[0] {
                                                        ...
                                                        '1'..='9' => state.registers.numbered.get(
                                                            command[0].to_digit(10).unwrap()
                                                                as usize
                                                                - 1,
                                                        ),
                                                        'a'..='z' => {
                                                            state.registers.named.get(&command[0])
                                                        }
                                                        _ => None,
                                                    };
                                                ...
                                                "y" => {
                                                    ...
                                                    let item_len: usize;
                                                    if command[0].is_ascii_lowercase() {
                                                        item_len = state.registers.yank_item(
                                                            &items,
                                                            Some(command[0]),
                                                            false,
                                                        );
                                                        ...
                                                        item_len = state.registers.yank_item(
                                                            &items,
                                                            Some(command[0].to_ascii_lowercase()),
                                                            true,
                                                        );
                                                    } else {
                                                        state.move_cursor(state.layout.y);
                                ...
                                match (code, modifiers) {
                                    ...
                                    (KeyCode::Enter, KeyModifiers::NONE) => {
                                        ...
                                        if commands.len() == 1 {
                                            match command {
                                                ...
                                                "reg" => {
                                                    ...
                                                    if state.layout.is_preview() {
                                                        ...
                                                        let (new_column, new_row) =
                                                            state.layout.update_column_and_row()?;
                                                        state.refresh(
                                                            new_column,
                                                            new_row,
                                                            state.layout.y,
                                                ...
                                                "config" => {
                                                    ...
                                                    {
                                                        if let Err(e) = state.chdir(
                                                            config_path.clone().parent().unwrap(),
                                                            Move::Jump,
                                                        ) {
                                                            print_warning(e, state.layout.y);
                                                        }
                                            ...
                                            if let Ok(target) =
                                                std::path::Path::new(commands[1]).normalize()
                                            {
                                                if target.exists() {
                                                    if let Err(e) = state
                                                        .chdir(&target.into_path_buf(), Move::Jump)
                                                    {
                                                        print_warning(e, state.layout.y);
                                                    }
                                                    break 'command;
                                        ...
                                        if std::env::set_current_dir(&state.current_dir).is_err() {
                                            err = Some("Changing current directory failed.");
                    ...
                    //exit by ZZ (chord resolved by the keymap)
                    Action::Quit => {
                        //In visual mode, this is disabled.
                        if state.v_start.is_some() {
                            continue;
                        }
                        if !state.match_vim_exit_behavior || state.export_lwd().is_ok() {
                            break 'main;
                        }
                    }

                    //exit by ZQ (chord resolved by the keymap)
